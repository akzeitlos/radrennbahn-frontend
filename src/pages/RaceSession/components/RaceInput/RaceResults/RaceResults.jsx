  ...
  return results.filter((r) => {
    ...
    if (filters.search) {
      ...
      if (
        ...
        !String(r.athleteNumber).includes(q)
      ) return false;
    }
...
const RaceResults = ({ race, modeSlug, results, entries }) => {
  ...
  const raceClasses      = race.raceClasses ?? [];
          ...
          <table className="race-results__table">
            <thead>
              <tr>
                ...
                <th className="race-results__col-classes">Klassen</th>
                {showPoints && <th className="race-results__col-pts">Punkte</th>}
                {showLaps && <th className="race-results__col-laps">{showPoints ? "Rd." : "Runden"}</th>}
